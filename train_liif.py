...
def train_warp(train_loader, model, optimizer):
    ...
    tq = tqdm(train_loader, leave=False, desc='train')
    for batch in tq:
        # Data preprocessing
        with torch.no_grad():
            gt = batch['inp'].cuda()
            ...
            lr_crop, m = warp_utils.get_input(gt, m_inv)
            ...
            # grid_raw: (2, N)
            # yi indicates the indices of valid positions in gt
            ...
            # Let's assume that the transformation is shared in a single batch
            coord = coord.repeat(gt.size(0), 1, 1)
        ...
        if num_samples > 0:
            gt_pix = gt_flat[..., yi[idx]]
        else:
            gt_pix = gt_flat[..., yi]

        gt_pix = gt_pix.transpose(-2, -1)
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            if n_gpus > 1 and (config.get('eval_bsize') is not None):
                ...
                model_ = model
            val_res = test.eval_warp_psnr(val_loader, model_,
                data_norm=config['data_norm'],
